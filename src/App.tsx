...
import { gsddSections, gsddIntro } from './gsdd-data';
import { DayPlan } from './types';
import { cn } from './lib/utils';
import { CheckCircle2, Circle, Clock, Layout, Bell, ChevronRight, Menu, X, Check, ExternalLink, ChevronDown, BookOpen, Star } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { useAuth } from './contexts/AuthContext';
import Header from './components/Header';
import GsddItemView from './components/GsddItemView';
import { saveProgress, loadProgress } from './lib/firestore';

type ViewMode = 'curriculum' | 'gsdd';

export default function App() {
  const { user } = useAuth();
  const [viewMode, setViewMode] = useState<ViewMode>('curriculum');
  const [selectedDay, setSelectedDay] = useState<number>(1);
  const [selectedGsddId, setSelectedGsddId] = useState<string>(gsddIntro.id);
  const [completedTopics, setCompletedTopics] = useState<Record<string, boolean>>({});
  const [isSidebarOpen, setIsSidebarOpen] = useState(true);
  const [showReminders, setShowReminders] = useState(false);
  const [collapsedPhases, setCollapsedPhases] = useState<Record<string, boolean>>({});
  const [gsddCollapsed, setGsddCollapsed] = useState(false);
  const [gsddBasicsCollapsed, setGsddBasicsCollapsed] = useState(false);
  const [gsddDesignsCollapsed, setGsddDesignsCollapsed] = useState(false);
  ...
  const toggleGsdd = async (id: string) => {
    const key = `gsdd-${id}`;
    await toggleTopic(key);
  };
  ...
  const calculateGsddProgress = () => {
    const allGsddIds = [
      gsddIntro.id,
      ...gsddSections.flatMap(s => s.items.map(item => item.id)),
    ];
    const total = allGsddIds.length;
    const completed = allGsddIds.filter(id => completedTopics[`gsdd-${id}`]).length;
    return { completed, total };
  };

  const gsddProgress = calculateGsddProgress();

  const selectGsddItem = (id: string) => {
    setSelectedGsddId(id);
    setViewMode('gsdd');
  ...
  const currentGsddItem = viewMode === 'gsdd'
    ? (selectedGsddId === gsddIntro.id
        ? gsddIntro
        : gsddSections.flatMap(s => s.items).find(i => i.id === selectedGsddId))
    : null;

  const currentPhaseLabel = viewMode === 'curriculum'
    ? curriculum.find(p => p.days.some(d => d.dayNumber === selectedDay))?.title.split(':')[0]
    : 'GSDD Reference';

  const currentPageLabel = viewMode === 'curriculum'
    ? `Day ${selectedDay}`
    : currentGsddItem?.title ?? '';
    ...
    <div className="flex h-screen bg-[#F8FAFC] text-slate-800 font-sans overflow-hidden">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6 scrollbar-thin scrollbar-thumb-slate-200">
                ...
                {!collapsedPhases[phase.id] && (
                  ...
                  >
                    <div className="space-y-1.5 pt-1">
                      {phase.days.map((day) => {
                        const isSelected = viewMode === 'curriculum' && selectedDay === day.dayNumber;
          ...
          {/* ── GSDD Reference Section ────────────────────────── */}
          <div className="space-y-2 border-t border-slate-200 pt-4">
            {/* Section Header */}
            <button
              onClick={() => setGsddCollapsed(p => !p)}
              className="w-full flex items-center justify-between text-left focus:outline-none group"
            >
              <div className="flex items-center gap-2 pl-1">
                <BookOpen size={13} className="text-amber-500 shrink-0" />
                <h3 className="text-xs font-bold uppercase text-amber-600 tracking-widest group-hover:text-amber-700 transition-colors">
                  GSDD Reference
                </h3>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-bold text-amber-500 bg-amber-50 px-1.5 py-0.5 rounded">
                  {gsddProgress.completed}/{gsddProgress.total}
                </span>
                <ChevronDown
                  size={14}
                  className={cn(
                    "text-slate-400 transition-transform duration-200",
                    gsddCollapsed ? "-rotate-90" : ""
                  )}
                />
              </div>
            </button>
              ...
              {!gsddCollapsed && (
                ...
                >
                  <div className="space-y-1.5 pt-1">
                    {/* Intro item */}
                    {(() => {
                      const isSelected = viewMode === 'gsdd' && selectedGsddId === gsddIntro.id;
                      const isRead = !!completedTopics[`gsdd-${gsddIntro.id}`];
                      return (
                        <button
                          onClick={() => selectGsddItem(gsddIntro.id)}
                          className={cn(
                            "w-full flex items-center gap-2.5 px-3 py-2 rounded text-left transition-all group",
                            isSelected
                        ...
                        >
                          <Star
                            ...
                            className={cn(
                              ...
                              isRead ? "text-amber-500 fill-amber-500" : "text-slate-300 group-hover:text-amber-400"
                            )}
                          />
                          <span className="text-sm truncate">Interview Guide</span>
                        </button>
                      );
                    })()}
                    ...
                    <div className="pt-1">
                      <button
                        onClick={() => setGsddBasicsCollapsed(p => !p)}
                        className="w-full flex items-center justify-between px-3 py-1 group focus:outline-none"
                      >
                        <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 group-hover:text-slate-500">Basics</span>
                        <ChevronDown size={11} className={cn("text-slate-300 transition-transform", gsddBasicsCollapsed ? "-rotate-90" : "")} />
                      </button>
                        ...
                        {!gsddBasicsCollapsed && (
                          ...
                          >
                            <div className="space-y-0.5 pt-0.5">
                              {gsddSections.find(s => s.id === 'basics')?.items.map((item) => {
                                const isSelected = viewMode === 'gsdd' && selectedGsddId === item.id;
                                const isRead = !!completedTopics[`gsdd-${item.id}`];
                                return (
                                  <button
                                    key={item.id}
                                    onClick={() => selectGsddItem(item.id)}
                                    className={cn(
                                      "w-full flex items-center gap-2.5 pl-5 pr-3 py-2 rounded text-left transition-all group",
                                      isSelected
                                        ? "bg-amber-50 text-amber-800 font-bold"
                                        : "hover:bg-slate-50 text-slate-500 hover:text-slate-900"
                                    )}
                                  >
                                    <div className={cn(
                                      "w-2 h-2 rounded-full shrink-0 transition-colors border",
                                      isRead
                    ...
                    <div className="pt-1">
                      <button
                        onClick={() => setGsddDesignsCollapsed(p => !p)}
                        className="w-full flex items-center justify-between px-3 py-1 group focus:outline-none"
                      >
                        <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 group-hover:text-slate-500">System Designs</span>
                        <ChevronDown size={11} className={cn("text-slate-300 transition-transform", gsddDesignsCollapsed ? "-rotate-90" : "")} />
                      </button>

                      <AnimatePresence initial={false}>
                        {!gsddDesignsCollapsed && (
                          <motion.div
                            initial={{ height: 0, opacity: 0 }}
                            animate={{ height: 'auto', opacity: 1 }}
                            exit={{ height: 0, opacity: 0 }}
                          ...
                          >
                            <div className="space-y-0.5 pt-0.5">
                              {gsddSections.find(s => s.id === 'designs')?.items.map((item) => {
                                const isSelected = viewMode === 'gsdd' && selectedGsddId === item.id;
                                const isRead = !!completedTopics[`gsdd-${item.id}`];
                                return (
                                  <button
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden bg-[#F8FAFC]">
        <header className="h-16 border-b border-slate-200 bg-white flex items-center px-4 md:px-8 shrink-0">
          ...
          <nav className="flex text-xs font-bold uppercase tracking-widest text-slate-400 items-center gap-2">
            ...
            <span className="text-slate-600 truncate max-w-[200px]">{currentPageLabel}</span>
          </nav>
          <Header />
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 scrollbar-thin scrollbar-thumb-slate-200">
          {/* ── GSDD View ────────────────────────────── */}
          {viewMode === 'gsdd' && currentGsddItem && (
            <GsddItemView
              item={currentGsddItem}
              isRead={!!completedTopics[`gsdd-${currentGsddItem.id}`]}
              onToggleRead={() => toggleGsdd(currentGsddItem.id)}
            />
          )}
